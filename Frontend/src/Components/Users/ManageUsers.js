...
const ManageUsers = () => {
        ...
        if (value.length >= 1) {
            ...
            try {
                const response = await axios.get(
                    `http://backend:8080/users/search?keyword=${value}`,
                    {
                        headers: {
    ...
    const getAllUsers = async () => {
        try {
            const token = localStorage.getItem("token");
            const response = await axios.get("http://backend:8080/admin/get-all-users", {
    ...
    const handleDelete = async (userId) => {
        try {
            const token = localStorage.getItem("token");
            const response = await axios.delete(`http://backend:8080/admin/delete/${userId}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    const handleUpdateUser = async () => {
        try {
            ...
            const response = await axios.put(
                `http://backend:8080/admin/update/${editingUser.id}`,
                safeUser,
                { headers: { Authorization: `Bearer ${token}` } }
            );
