...
const ManageUsers = () => {
        ...
        if (value.length >= 1) {
            ...
            try {
                const response = await axios.get(
                    `http://13.233.99.93:8080/users/search?keyword=${value}`,
                    {
                        headers: {
    ...
    const createUser = async (e) => {
        ...
        try {
            ...
            const response = await axios.post("http://13.233.99.93:8080/auth/register", {
                name,
                email,
                password,
                role,
    ...
    const getAllUsers = async () => {
        try {
            const token = localStorage.getItem("token");
            const response = await axios.get("http://13.233.99.93:8080/admin/get-all-users", {
    ...
    const handleDelete = async (userId) => {
        try {
            const token = localStorage.getItem("token");
            const response = await axios.delete(`http://13.233.99.93:8080/admin/delete/${userId}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    const handleUpdateUser = async () => {
        try {
            ...
            const response = await axios.put(
                `http://13.233.99.93:8080/admin/update/${editingUser.id}`,
                safeUser,
                { headers: { Authorization: `Bearer ${token}` } }
            );
