...
const ForgotPassword = () => {
    ...
    const handleEmailSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post("http://54.152.143.62:8080/auth/email/reset-password/send-otp", { email });
    ...
    const handlePasswordReset = async (e) => {
        ...
        try {
            ...
            const response = await axios.post("http://54.152.143.62:8080/auth/email/reset-password/verify-otp", payload);
            const { statusCode, message } = response.data;
