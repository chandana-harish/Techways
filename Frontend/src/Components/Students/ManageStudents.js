...
const ManageStudents = () => {
    ...
    const handleCreateStudent = async (e) => {
        ...
        try {
            const token = localStorage.getItem("token");
            const response = await axios.post("http://54.144.95.44:8080/student/register", newStudent, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    const handleUpdateStudent = async (imageFile) => {
        try {
            ...
            if (imageFile) {
                ...
                const uploadRes = await fetch("http://54.144.95.44:8080/cloudinary/upload", {
                    method: "POST",
                    body: formData,
                });
            ...
            const response = await axios.put(
                `http://54.144.95.44:8080/student/update/${editingStudent.id}`,
                payload,
                { headers: { Authorization: `Bearer ${token}` } }
            );
    ...
    const confirmDelete = async (studentId) => {
        try {
            const token = localStorage.getItem("token");
            const response = await axios.delete(`http://54.144.95.44:8080/student/delete/${studentId}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
