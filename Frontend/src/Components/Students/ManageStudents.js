...
const ManageStudents = () => {
    ...
    const getAllStudents = async () => {
        try {
            ...
            const response = await axios.get("http://backend:8080/student/get-all", {
    ...
    const handleCreateStudent = async (e) => {
        ...
        try {
            const token = localStorage.getItem("token");
            const response = await axios.post("http://backend:8080/student/register", newStudent, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    const handleUpdateStudent = async (imageFile) => {
        try {
            ...
            if (imageFile) {
                ...
                const uploadRes = await fetch("http://backend:8080/cloudinary/upload", {
                    method: "POST",
                    body: formData,
                });
            ...
            const response = await axios.put(
                `http://backend:8080/student/update/${editingStudent.id}`,
                payload,
                { headers: { Authorization: `Bearer ${token}` } }
            );
    ...
    const confirmDelete = async (studentId) => {
        try {
            const token = localStorage.getItem("token");
            const response = await axios.delete(`http://backend:8080/student/delete/${studentId}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
        ...
        if (value.length >= 1) {
            ...
            try {
                const response = await axios.get(
                    `http://backend:8080/student/search?keyword=${value}`,
                    {
                        headers: {
